...
async function loadSession(){if(!sessionId){renderMessages([]);return}try{const res=await fetch(`/api/podcast-asr/qa/sessions/${encodeURIComponent(sessionId)}`,{cache:'no-store'});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.detail||'会话不存在');const session=data.session||{};if(session.episode_id&&episodeSelect&&[...episodeSelect.options].some(o=>o.value===session.episode_id))episodeSelect.value=session.episode_id;if(session.scope)setScope(session.scope);renderMessages(session.messages||[])}catch(e){sessionId='';localStorage.removeItem(SESSION_KEY);renderMessages([])}}
...
async function poll(taskId){try{const res=await fetch(`/api/podcast-asr/qa/questions/${encodeURIComponent(taskId)}`,{cache:'no-store'});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.detail||'查询失败');const task=data.task;if(task.session_id&&!sessionId){sessionId=task.session_id;localStorage.setItem(SESSION_KEY,sessionId)}if(['queued','running'].includes(task.status)){setBusy(true,'Agent 正在检索摘要、大纲和转写原文…');return}clearInterval(pollTimer);pollTimer=null;activeTask='';localStorage.removeItem(TASK_KEY);setBusy(false);if(task.status==='completed'){await loadSession();setScope(task.scope);status.textContent='回答完成 · '+(task.scope==='all'?'全部播客':episodeSelect?.selectedOptions[0]?.textContent||'当前播客')}else{status.textContent='回答失败：'+(task.error||'未知错误')}}catch(e){setBusy(false);status.textContent='状态查询失败：'+e.message}}
function startPolling(taskId){if(pollTimer)clearInterval(pollTimer);activeTask=taskId;localStorage.setItem(TASK_KEY,taskId);setBusy(true,'Agent 正在启动…');poll(taskId);pollTimer=setInterval(()=>poll(taskId),2000)}
scopeButtons.forEach(button=>button.addEventListener('click',()=>setScope(button.dataset.qaScope)));episodeSelect?.addEventListener('change',()=>setScope(scope));form.addEventListener('submit',async event=>{event.preventDefault();const question=input.value.trim();if(!question)return;if(scope==='episode'&&!episodeSelect?.value){status.textContent='请先选择一个播客';return}setBusy(true,'正在创建后台问答任务…');try{const payload={question,scope,episode_id:scope==='episode'?episodeSelect.value:null,session_id:sessionId||null};const res=await fetch('/api/podcast-asr/qa/questions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.detail||'创建失败');sessionId=data.task.session_id;localStorage.setItem(SESSION_KEY,sessionId);input.value='';renderMessages([...messages,{role:'user',content:question}]);startPolling(data.task.task_id)}catch(e){setBusy(false);status.textContent='提交失败：'+e.message}});newButton?.addEventListener('click',()=>{sessionId='';activeTask='';localStorage.removeItem(SESSION_KEY);localStorage.removeItem(TASK_KEY);if(pollTimer)clearInterval(pollTimer);pollTimer=null;setBusy(false);renderMessages([]);setScope(root.dataset.defaultScope||'all')});setScope(scope);loadSession().then(()=>{if(activeTask)startPolling(activeTask)});
})();
""".strip()
