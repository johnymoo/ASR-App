...
QA_CSS = """
.import,.qa-panel,.task-status,.section-head>*{min-width:0}.task-status,.qa-bubble,.qa-citation,.page-header p{overflow-wrap:anywhere}.qa-citation span{display:block}
.qa-panel{display:grid;gap:16px;margin:24px 0}.content>.qa-panel{margin:0}.qa-head{display:flex;align-items:start;justify-content:space-between;gap:16px}.qa-controls{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.segmented{display:inline-flex;border:1px solid var(--line-strong);border-radius:6px;overflow:hidden}.segmented button{min-height:34px;border:0;border-right:1px solid var(--line-strong);background:var(--panel);color:var(--muted);padding:5px 12px;cursor:pointer}.segmented button:last-child{border-right:0}.segmented button.active{background:var(--ink);color:var(--panel);font-weight:600}.qa-episode{min-width:220px;max-width:480px;height:36px;border:1px solid var(--line-strong);border-radius:6px;background:var(--panel);color:var(--ink);padding:0 10px}.qa-thread{display:grid;gap:14px;min-height:96px;max-height:560px;overflow:auto;padding:16px;border:1px solid var(--line);border-radius:6px;background:var(--panel2)}.qa-empty{align-self:center;color:var(--muted);text-align:center}.qa-message{max-width:88%;display:grid;gap:6px}.qa-message.user{justify-self:end}.qa-message.assistant{justify-self:start}.qa-bubble{padding:10px 12px;border:1px solid var(--line);border-radius:6px;background:var(--panel);white-space:pre-wrap}.qa-message.user .qa-bubble{background:var(--blue);border-color:var(--blue);color:#fff}.qa-label{font-size:12px;color:var(--muted)}.qa-citations{display:grid;gap:6px}.qa-citation{display:block;padding:8px 10px;border-left:3px solid var(--blue);background:var(--panel);color:var(--ink)}.qa-citation small{display:block;color:var(--muted)}.qa-compose{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}.qa-compose textarea,.settings-form input,.settings-form textarea{width:100%;min-width:0;border:1px solid var(--line-strong);border-radius:6px;background:var(--panel);color:var(--ink);padding:10px 12px;resize:vertical}.qa-compose textarea{min-height:64px}.qa-compose .btn{align-self:stretch;min-width:88px}.settings-form{display:grid;gap:18px}.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.settings-field{display:grid;gap:6px}.settings-field label{font-weight:600}.settings-field small{color:var(--muted)}.settings-field.full{grid-column:1/-1}.settings-status{padding:12px;border:1px solid var(--line);border-radius:6px;background:var(--panel2)}@media(max-width:599px){.qa-head{display:grid}.qa-controls{align-items:stretch}.segmented{width:100%}.segmented button{flex:1}.qa-episode{max-width:none;width:100%}.qa-message{max-width:96%}.qa-compose{grid-template-columns:1fr}.qa-compose .btn{min-height:42px}.settings-grid{grid-template-columns:1fr}.settings-field.full{grid-column:auto}}
""".strip()

QA_JS = r"""
(()=>{const root=document.getElementById('podcastQa');if(!root)return;const thread=root.querySelector('[data-qa-thread]'),form=root.querySelector('[data-qa-form]'),input=root.querySelector('[data-qa-input]'),send=root.querySelector('[data-qa-send]'),episodeSelect=root.querySelector('[data-qa-episode]'),scopeButtons=[...root.querySelectorAll('[data-qa-scope]')],status=root.querySelector('[data-qa-status]'),newButton=root.querySelector('[data-qa-new]');const SESSION_KEY='podcastQa.sessionId',TASK_KEY='podcastQa.activeTaskId';let sessionId=localStorage.getItem(SESSION_KEY)||'',activeTask=localStorage.getItem(TASK_KEY)||'',pollTimer=null,scope=root.dataset.defaultScope||'all',messages=[],conversationEpoch=0;
const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
function setScope(next){scope=next==='episode'?'episode':'all';scopeButtons.forEach(b=>b.classList.toggle('active',b.dataset.qaScope===scope));if(episodeSelect)episodeSelect.hidden=scope!=='episode';root.dataset.scope=scope;status.textContent=scope==='all'?'回答范围：全部已发布播客':'回答范围：'+(episodeSelect?.selectedOptions[0]?.textContent||'当前播客')}
function citationHtml(c){const time=c.timestamp?`${c.timestamp}${c.timestamp_approximate?' 左右':''}`:'摘要 / 大纲';return `<a class="qa-citation" href="${esc(c.url||'#')}" target="_blank" rel="noopener"><strong>${esc(c.episode_title||'播客')}</strong><small>${esc(time)} · ${esc(c.source_type||'source')}</small><span>${esc(c.text||'')}</span></a>`}
function messageHtml(message){const role=message.role==='user'?'user':'assistant';const citations=role==='assistant'&&(message.citations||[]).length?`<div class="qa-citations">${message.citations.map(citationHtml).join('')}</div>`:'';return `<article class="qa-message ${role}"><div class="qa-label">${role==='user'?'你':'播客 Agent'}</div><div class="qa-bubble">${esc(message.content||'')}</div>${citations}</article>`}
function renderMessages(next){messages=next||[];thread.innerHTML=messages.length?messages.map(messageHtml).join(''):'<div class="qa-empty">选择范围后，可以询问书名、观点、人物或某段讨论。</div>';thread.scrollTop=thread.scrollHeight}
async function loadSession(epoch=conversationEpoch){if(!sessionId){renderMessages([]);return}try{const res=await fetch(`/api/podcast-asr/qa/sessions/${encodeURIComponent(sessionId)}`,{cache:'no-store'});const data=await res.json();if(epoch!==conversationEpoch)return;if(!res.ok||!data.ok)throw new Error(data.detail||'会话不存在');const session=data.session||{};if(session.episode_id&&episodeSelect&&[...episodeSelect.options].some(o=>o.value===session.episode_id))episodeSelect.value=session.episode_id;if(session.scope)setScope(session.scope);renderMessages(session.messages||[])}catch(e){if(epoch!==conversationEpoch)return;sessionId='';localStorage.removeItem(SESSION_KEY);renderMessages([])}}
function setBusy(busy,text=''){send.disabled=busy;input.disabled=busy;send.textContent=busy?'回答中…':'发送';if(text)status.textContent=text}
async function poll(taskId,epoch=conversationEpoch){try{const res=await fetch(`/api/podcast-asr/qa/questions/${encodeURIComponent(taskId)}`,{cache:'no-store'});const data=await res.json();if(epoch!==conversationEpoch)return;if(!res.ok||!data.ok)throw new Error(data.detail||'查询失败');const task=data.task;if(task.session_id&&!sessionId){sessionId=task.session_id;localStorage.setItem(SESSION_KEY,sessionId)}if(['queued','running'].includes(task.status)){setBusy(true,'Agent 正在检索摘要、大纲和转写原文…');return}clearInterval(pollTimer);pollTimer=null;activeTask='';localStorage.removeItem(TASK_KEY);setBusy(false);if(task.status==='completed'){await loadSession(epoch);if(epoch!==conversationEpoch)return;setScope(task.scope);status.textContent='回答完成 · '+(task.scope==='all'?'全部播客':episodeSelect?.selectedOptions[0]?.textContent||'当前播客')}else{status.textContent='回答失败：'+(task.error||'未知错误')}}catch(e){if(epoch!==conversationEpoch)return;setBusy(false);status.textContent='状态查询失败：'+e.message}}
function startPolling(taskId){if(pollTimer)clearInterval(pollTimer);const epoch=conversationEpoch;activeTask=taskId;localStorage.setItem(TASK_KEY,taskId);setBusy(true,'Agent 正在启动…');poll(taskId,epoch);pollTimer=setInterval(()=>poll(taskId,epoch),2000)}
scopeButtons.forEach(button=>button.addEventListener('click',()=>setScope(button.dataset.qaScope)));episodeSelect?.addEventListener('change',()=>setScope(scope));input.addEventListener('keydown',event=>{if(event.key!=='Enter'||event.shiftKey||event.isComposing)return;event.preventDefault();if(!send.disabled)form.requestSubmit()});form.addEventListener('submit',async event=>{event.preventDefault();const question=input.value.trim();if(!question)return;if(scope==='episode'&&!episodeSelect?.value){status.textContent='请先选择一个播客';return}setBusy(true,'正在创建后台问答任务…');const epoch=conversationEpoch;try{const payload={question,scope,episode_id:scope==='episode'?episodeSelect.value:null,session_id:sessionId||null};const res=await fetch('/api/podcast-asr/qa/questions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(epoch!==conversationEpoch)return;if(!res.ok||!data.ok)throw new Error(data.detail||'创建失败');sessionId=data.task.session_id;localStorage.setItem(SESSION_KEY,sessionId);input.value='';renderMessages([...messages,{role:'user',content:question}]);startPolling(data.task.task_id)}catch(e){if(epoch!==conversationEpoch)return;setBusy(false);status.textContent='提交失败：'+e.message}});newButton?.addEventListener('click',()=>{conversationEpoch++;sessionId='';activeTask='';localStorage.removeItem(SESSION_KEY);localStorage.removeItem(TASK_KEY);if(pollTimer)clearInterval(pollTimer);pollTimer=null;setBusy(false);renderMessages([]);status.textContent='新对话 · '+(scope==='all'?'全部播客':episodeSelect?.selectedOptions[0]?.textContent||'当前播客');input.focus()});setScope(scope);loadSession().then(()=>{if(activeTask)startPolling(activeTask)});
})();
""".strip()

SETTINGS_JS = r"""
(()=>{const form=document.getElementById('agentSettings'),status=document.getElementById('settingsStatus');if(!form)return;const fields=['api_base','api_key','model','system_prompt','temperature','timeout_seconds'];function setStatus(text,error=false){status.textContent=text;status.style.color=error?'var(--red)':''}async function load(){setStatus('正在读取配置…');try{const res=await fetch('/api/podcast-asr/qa/config',{cache:'no-store'});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.detail||'读取失败');const c=data.config;fields.forEach(name=>{const el=form.elements[name];if(!el)return;if(name==='api_key'){el.value='';el.placeholder=c.api_key_set?`已保存 ${c.api_key_masked}，留空表示不修改`:'未设置；本地无鉴权接口可留空'}else el.value=c[name]??''});setStatus(c.configured?'配置已加载':'配置不完整',!c.configured)}catch(e){setStatus('读取失败：'+e.message,true)}}form.addEventListener('submit',async event=>{event.preventDefault();const payload={};fields.forEach(name=>{const value=form.elements[name].value;if(name==='api_key'&&!value)return;payload[name]=['temperature'].includes(name)?Number(value):name==='timeout_seconds'?Number(value):value});setStatus('正在保存…');try{const res=await fetch('/api/podcast-asr/qa/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.detail||'保存失败');setStatus('已保存，后续问题将使用新配置。');await load()}catch(e){setStatus('保存失败：'+e.message,true)}});load()})();
""".strip()
...
def render_qa_panel(
    ...
    for episode in episodes:
        ...
        if not episode_id:
            continue
        selected = " selected" if episode_id == current_episode_id else ""
        options.append(f'<option value="{esc(episode_id)}"{selected}>{esc(episode.get("title") or episode_id)}</option>')
    if not options and current_episode_id:
        options.append(f'<option value="{esc(current_episode_id)}" selected>当前播客</option>')
    episode_disabled = " disabled" if not options else ""
    return f'''<section class="panel qa-panel" id="podcastQa" data-default-scope="{esc(default_scope)}" data-current-episode="{esc(current_episode_id)}"><div class="qa-head"><div><h2>问播客 Agent</h2><p class="dim">依据已发布的 summary、官方大纲和转写原文回答，并附原文定位。</p></div><div class="actions"><button class="btn" type="button" data-qa-new>新对话</button><a class="btn" href="/static/podcast-asr/settings/index.html">Agent 设置</a></div></div><div class="qa-controls"><div class="segmented" aria-label="问答范围"><button type="button" data-qa-scope="episode">当前播客</button><button type="button" data-qa-scope="all">全部播客</button></div><select class="qa-episode" data-qa-episode aria-label="选择播客"{episode_disabled}>{''.join(options)}</select><span class="dim" data-qa-status aria-live="polite"></span></div><div class="qa-thread" data-qa-thread aria-live="polite"><div class="qa-empty">选择范围后，可以询问书名、观点、人物或某段讨论。</div></div><form class="qa-compose" data-qa-form><textarea data-qa-input maxlength="4000" aria-label="向播客 Agent 提问" placeholder="例如：奥德赛这期推荐了哪本书？"></textarea><button class="btn primary" type="submit" data-qa-send>发送</button></form></section>'''


def render_settings_page() -> str:
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Agent 设置｜Podcast ASR Studio</title><style>{CSS}{QA_CSS}</style></head><body><header class="appbar"><nav class="appbar-inner"><div class="brand">Podcast ASR Studio<small>Agent 设置</small></div><div class="nav"><a href="../index.html">播客转写</a><a href="/static/meeting-asr/index.html">会议转写</a><a href="index.html" aria-current="page">Agent 设置</a></div></nav></header><main class="shell"><section class="page-header"><h1>播客 Agent 设置</h1><p>配置一个 OpenAI-compatible Agent。设置保存在服务端，API key 不会写入静态页面。</p></section><section class="panel"><form id="agentSettings" class="settings-form"><div class="settings-grid"><div class="settings-field full"><label for="apiBase">API 地址</label><input id="apiBase" name="api_base" type="url" required placeholder="http://127.0.0.1:8004/v1"><small>OpenAI-compatible API base，系统会调用 /chat/completions。</small></div><div class="settings-field"><label for="model">模型</label><input id="model" name="model" required placeholder="qwen3.6-35b-fp8"></div><div class="settings-field"><label for="apiKey">API key</label><input id="apiKey" name="api_key" type="password" autocomplete="new-password"><small>留空表示保留已保存的 key。</small></div><div class="settings-field"><label for="temperature">Temperature</label><input id="temperature" name="temperature" type="number" min="0" max="2" step="0.05" required></div><div class="settings-field"><label for="timeout">超时（秒）</label><input id="timeout" name="timeout_seconds" type="number" min="5" max="1800" step="1" required></div><div class="settings-field full"><label for="systemPrompt">System prompt</label><textarea id="systemPrompt" name="system_prompt" rows="8" required></textarea><small>无论如何配置，后端仍只向 Agent 提供当前播客库中的 summary、大纲和原文证据。</small></div></div><div id="settingsStatus" class="settings-status" role="status" aria-live="polite"></div><div class="actions"><button class="btn primary" type="submit">保存配置</button><a class="btn" href="../index.html#podcastQa">返回问答</a></div></form></section></main><script>{SETTINGS_JS}</script></body></html>'''
...
def render_episode(ep: Episode) -> tuple[str, str, dict[str, Any]]:
    ...
    qa_panel = render_qa_panel(
        [{"episode_id": trans.get("episode_id"), "title": title}],
        default_scope="episode",
        current_episode_id=str(trans.get("episode_id") or ""),
    )
    ...
    report_html = report_html.replace(f"<style>{CSS}</style>", f"<style>{CSS}{QA_CSS}</style>")
    report_html = report_html.replace(
        '<a href="/healthz">服务状态</a>',
        '<a href="/static/podcast-asr/settings/index.html">Agent 设置</a><a href="/healthz">服务状态</a>',
    )
    report_html = report_html.replace(
        '<div class="actions"><a class="btn primary" href="#summary">读总结</a>',
        '<div class="actions"><a class="btn primary" href="#podcastQa">问本期</a><a class="btn" href="#summary">读总结</a>',
    )
    report_html = report_html.replace(
        '<aside class="panel toc" aria-label="报告章节">',
        '<aside class="panel toc" aria-label="报告章节"><a href="#podcastQa">问播客</a>',
    )
    report_html = report_html.replace('<div class="content">', f'<div class="content">{qa_panel}', 1)
    report_html = report_html.replace("</body></html>", f"<script>{QA_JS}</script></body></html>")
